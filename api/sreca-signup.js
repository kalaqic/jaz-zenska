// Vercel Serverless Function for Sreca Webinar Signup
const fetch = require('node-fetch');

const GETRESPONSE_API_KEY = process.env.GETRESPONSE_API_KEY;
const GETRESPONSE_API_URL = 'https://api.getresponse.com/v3/contacts';
const SRECA_CAMPAIGN_ID = 'f5M9D'; // Sreca webinar campaign
...
module.exports = async function handler(req, res) {
    ...
    try {
        ...
        const contactData = {
            ...
            name: name,
            campaign: {
                campaignId: SRECA_CAMPAIGN_ID
            }
        };

        console.log('=== Creating sreca webinar signup contact ===');
        console.log('Email:', email);
        console.log('Name:', name);
        console.log('Campaign ID:', SRECA_CAMPAIGN_ID);
        ...
        if (response.status === 409) {
            ...
            if (isAlreadyAdded) {
                ...
                return res.status(500).json({ 
                    error: data.message || data.error || 'Failed to sign up for sreca webinar',
        ...
        // Success (200, 201, 202)
        console.log('Sreca webinar signup contact created successfully');
        
        return res.status(200).json({ 
            success: true, 
            message: 'Successfully signed up for sreca webinar',
            data: data
        });
    } catch (error) {
